...
def load_and_visualize_pcds(raw_pcd_path, control_pcd_path, fittedSuface_pcd_path):
    ...
    def key_show_fitted_callback(vis):
        vis.remove_geometry(fitted_pcd)
        return True
    
    def key_show_control_callback(vis):
        vis.remove_geometry(control_pcd)
        return True
    ...
    vis.add_geometry(control_pcd)
    vis.add_geometry(fitted_pcd)

    # run vis
    vis.register_key_callback(ord("1"), key_show_orgi_callback)
    vis.register_key_callback(ord("2"), key_show_fitted_callback)
    vis.register_key_callback(ord("3"), key_show_control_callback)
