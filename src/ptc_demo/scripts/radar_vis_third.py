...
def load_and_visualize_pcds(original_pcd_path, segmented_pcd_path, bspline_pcd_path):
    ...
    # large_points_repeated = np.repeat(large_points, 20, axis=0)  # 这里的 5 表示重复次数
    ...
    # 设置渲染选项
    vis.get_render_option().point_size = point_size  # 设置点的大小
    ...
    # 定义键盘回调函数
    def change_point_size(vis, point_size_index):
        def callback(vis):
            nonlocal point_size
            point_size[point_size_index] += 1  # 增大点大小
            render_option = vis.get_render_option()
            render_option.point_size = point_size[point_size_index]
            vis.update_renderer()
        return callback
    ...
    # 为每个点云设置不同的快捷键来改变点大小
    vis.register_key_callback(ord("1"), change_point_size(vis, 0))
    vis.register_key_callback(ord("2"), change_point_size(vis, 1))
    vis.register_key_callback(ord("3"), change_point_size(vis, 2))
